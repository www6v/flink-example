...
/**
  ...
  */
class RoomTimeAssigner
//    extends BoundedOutOfOrdernessTimestampExtractor[SensorReading](Time.seconds(5)) {
  extends BoundedOutOfOrdernessTimestampExtractor[RoomReading](Time.seconds(5)) {
  /** Extracts timestamp from SensorReading. */
//  override def extractTimestamp(r: SensorReading): Long = r.timestamp
  override def extractTimestamp(r: RoomReading): Long = r.timestamp
}
